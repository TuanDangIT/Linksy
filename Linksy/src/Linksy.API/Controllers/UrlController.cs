...
namespace Linksy.API.Controllers
{
    ...
    public class UrlController : BaseController
    {
        ...
        [HttpGet]
        public async Task<ActionResult<ApiResponse<BrowseUrlsResponse>>> BrowseUrls([FromQuery] BrowseUrls command, CancellationToken cancellationToken)
            => Ok(await _mediator.Send(command, cancellationToken));

        [HttpPost]
        public async Task<ActionResult<ApiResponse<ShortenUrlResponse>>> ShortentUrl([FromBody] ShortenUrl command, CancellationToken cancellationToken)
        {
            ...
            [FromQuery] string? umtMedium, [FromQuery] string? umtCampaign, [FromQuery] bool? isQrCode, [FromQuery] bool? isBarcode, CancellationToken cancellationToken)
        {
            ...
            {
                result = await _mediator.Send(new RedirectToOriginalUrl(code, ipAddress, new UmtParameterDto(umtSource, umtMedium, umtCampaign), isQrCode, isBarcode), cancellationToken);
            }
            else
            {
                result = await _mediator.Send(new RedirectToOriginalUrl(code, ipAddress, null, isQrCode, isBarcode), cancellationToken);
            }
        ...
        {
            await _mediator.Send(new SetActiveStatus(urlId, false), cancellationToken);
            return NoContent();
        }
